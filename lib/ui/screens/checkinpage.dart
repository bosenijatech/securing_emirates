...
class _CheckinpageState extends State<Checkinpage> {
  ...
  /// Load shifts already checked in today
Future<void> _loadDisabledShifts() async {
  var box = Hive.box('shiftBox');
  String today = _today();

  // stored: {"lastCheckinDate": "19-09-2025"}
  String? lastCheckinDate = box.get('lastCheckinDate');

  setState(() {
    if (lastCheckinDate == today) {
      // disable all shifts if already checked in today
      disabledShiftIds = shiftMap.values.toSet();
    } else {
...
/// 🟢 Check if already checked in today (any shift)
Future<bool> hasCheckedInToday(int shiftId) async {
  var box = Hive.box('shiftBox');
  String today = _today();
  String? lastCheckinDate = box.get('lastCheckinDate');
...
/// 🟢 Mark checked in (only once per day)
Future<void> markCheckedIn(int shiftId) async {
  var box = Hive.box('shiftBox');
  String today = _today();

  await box.put('lastCheckinDate', today);

  setState(() {
    // disable all shifts for the rest of the day
    disabledShiftIds = shiftMap.values.toSet();
    usershift = null;
  });
}
